...
printf(strftime ("%Y-%m-%d %H:%M:%S", localtime (time ())));
printf('\ncnnConvolve started\n');


%% Use only the first 8 images for testing
convImages = trainImages(:, :, :, 1:15); 
...
if ~isequal(pooledFeatures, expectedMatrix)
    ...
    disp('Got');
    disp(pooledFeatures);
	error('Your pooling code failed ');
else
...
if debug
	numTrainImages = 10;
	numTestImages = numTrainImages;
	trainImages = trainImages(:, :, :, 1:numTrainImages);
	trainLabels = trainLabels(1:numTrainImages);
	testImages = testImages(:, :, :, 1:numTrainImages);
	testLabels = testLabels(1:numTrainImages);
end
...
printf(strftime ("%Y-%m-%d %H:%M:%S", localtime (time ())));
printf('\nConvolving and pooling test images done \n');
